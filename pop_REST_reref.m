...
% --- Executes on button press in Calculate_LeadField_pushbutton2.
function Calculate_LeadField_pushbutton2_Callback(hObject, eventdata, handles)
% hObject    handle to Calculate_LeadField_pushbutton2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[ProgramPath, ~, ~] = fileparts(which('REST_refer.m'));
eval(['!',[ProgramPath,filesep],'LeadField.exe']);
...
try EEG = evalin('base','EEG');
    if ~isempty(EEG.data)
        ...
        if ~isempty(handles.cfg.InputLeadfieldDir)
            ...
            errordlg('Please select lead field file first!!!!','Data Error');
            return
        end
    ...
    errordlg('Failed to find EEG data OR other mistakes (e.g. Inputs)!!!','Data Error');
    return;
end;
...
msgbox({'Re-referencing to REST steps: ';...
        ...
        '   [3] Select a lead field file (has been calculated and saved as *.txt/*.xls/*.xlsx/*.dat):';...
        '        if you want to calcuate a new leadfield matrix file, press the button ''Calculate Lead Field'' below';...
        '        1. File-->Load Electrode File: *.txt ASCII file; x (left ear is the -x axis),y (the nasion is the +y axis),z (the vertex is the +z axis) position in three columns';...
        '        2. File-->Calculate Lead Field, it may take a few minutes that depends on the size of the matrix and the computer.';...
        '        3. -->Lead Field calculation completed. Now the lead field matrix is saved as ''Lead_Field.dat'' in the directory of Electrode File. Here the Lead_Field is *.dat format (e.g. Lead_Field.dat in the file,sources X channels). ';...
        '   [4] Press button ''Run'';';...
        '   [5] Press button ''OK'' to save the re-referencing data to workspace (ALLEEG).In EEGLAB, click ''Datasets''-->''*_REST''';...
        },'Help');
